...
SELECT
    ...
    IFNULL(ddm.total_sum, 0.00) as ddm_total_sum,
...
LEFT JOIN (SELECT inn, total_sum from derivative_debt_meta) AS ddm ON ddm.inn = meta.inn
WHERE meta.region = ? AND resolutions.is_derivative_debt >= 1 AND resolutions.is_archive = 0
group by meta.inn
`
